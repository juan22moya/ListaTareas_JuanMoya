                ...
                <section class="todoapp">
                    ...
                    <footer class=("footer", hidden_class)>
                        ...
                        <button class="clear-completed" onclick=self.link.callback(|_| Msg::ClearCompleted)>
                            { format!("Limpiar Completados ({})", self.state.total_completed()) }
                        </button>
                    </footer>
                </section>
                <footer class="info">
                    <p>{ "Doble click para modificar" }</p>
...
impl<'a> Into<Href> for &'a Filter {
    fn into(self) -> Href {
        match *self {
            Filter::Todos => "#/".into(),
            Filter::Activos => "#/active".into(),
            Filter::Completados => "#/completed".into(),
        }
    }
}
...
impl Filter {
    fn fit(&self, entry: &Entry) -> bool {
        match *self {
            Filter::Todos => true,
            Filter::Activos => !entry.completed,
            Filter::Completados => entry.completed,
        }
    }
}
...
impl State {
    ...
    fn total_completed(&self) -> usize {
        self.entries
            .iter()
            .filter(|e| Filter::Completados.fit(e))
            .count()
    }
